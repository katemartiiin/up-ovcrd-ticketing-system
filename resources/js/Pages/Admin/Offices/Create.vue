...
export default {
    ...
    setup() {
        ...
        const addUser = () => {
            payload.users.push(selectedUser.value)
            selectedUser.value = ''
        }
        ...
        const addProcess = () => {
            payload.processes.push(selectedProcess.value)
            selectedProcess.value = ''
        }

        const removeProcess = (index) => {
            payload.processes.splice(index, 1)
        }
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <Breadcrumb parentTitle="Sections" :parentLink="route('admin.offices.index')" currentTitle="Add" />
        </template>
            ...
            <div class="w-full flex flex-wrap mb-5">
                ...
                <div class="w-full md:w-3/4 my-2">
                    <div class="w-full flex flex-wrap mb-3">
                        <input v-model="payload.name" type="text" class="w-full border-gray-300 rounded-md mr-1" placeholder="Section Name">
                        <ErrorText v-if="errors.name" :message="errors.name[0]" />
                    </div>
                    <div class="bg-white pt-5 pb-10 px-10 md:mr-1.5 rounded-md">
                        <div class="w-full flex flex-wrap mt-5 mb-7 pb-5">
                            <div class="w-full md:w-1/2 py-3 md:py-0 md:pr-2">
                                ...
                                <input v-model="payload.short_code" type="text" class="w-full border-gray-300 rounded-md mr-1 p-2" placeholder="Short Code">
                                <ErrorText v-if="errors.short_code" :message="errors.short_code[0]" />
                            </div>
                            <div class="w-full md:w-1/2 py-3 md:py-0">
                                <label class="block text-up-green font-semibold mb-2">POC</label>
                                <select v-model="payload.head_id" id="countries" class="bg-gray-50 border border-gray-300 text-sm text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    <option disabled selected value="">Select user</option>
                                    <option v-for="user in users" :key="user.id" :value="user.id">{{ user.name }}</option>
                                </select>
                            </div>
                        </div>
                        <!-- <div class="w-full flex flex-wrap mt-5 mb-7 pb-5 text-lg">
                            <label class="block text-up-green font-semibold mb-2">POC</label>
                            <select v-model="payload.head_id" id="countries" class="bg-gray-50 border border-gray-300 text-sm text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2">
                                <option disabled selected value="">Select user</option>
                                <option v-for="user in users" :key="user.id" :value="user.id">{{ user.name }}</option>
                            </select>
                        </div> -->
                        <div class="w-full mt-3 mb-10 pb-10 text-lg">
                            ...
                            <textarea v-model="payload.description" class="w-full bg-gray-50 rounded-md border border-gray-300 text-sm" placeholder="Enter description" rows="5"></textarea>
                        </div>
                    </div>
                    <div class="w-full flex items-end justify-end px-2 pt-5">
                        <button @click="createOffice" type="button" class="bg-up-green-light hover:bg-up-green text-white px-5 py-2 rounded-md transition duration-200 ease-in-out">Create</button>
                    </div>
                </div>
                ...
                <div class="w-full md:w-1/4 mt-2">
                    ...
                    <div class="bg-white p-5 md:ml-1.5 rounded-md mb-5">
                        <div class="w-full mt-3">
                            ...
                            <div class="w-full flex flex-wrap mt-5 mb-3 bg-gray-100 rounded-md px-5 py-3 max-h-64 overflow-y-scroll">
                                <template v-if="payload.users.length">
                                    <div v-for="(user, index) in payload.users" :key="user.id" class="w-full flex grid grid-cols-2 gap-2 text-sm my-3">
                                        <div>
                                            <p>{{ user.name }}</p>
                                        </div>
                                        <div class="text-right">
                                            <button @click="removeUser(index)" type="button" class="text-red-600"><font-awesome-icon icon="fa-solid fa-minus" /></button>
                                        </div>
                                    </div>
                                </template>
                                <template v-else>
                    ...
                    <div class="bg-white p-5 md:ml-1.5 rounded-md">
                        <div class="w-full mt-3">
                            ...
                            <div class="w-full flex flex-wrap mt-5 mb-3 bg-gray-100 rounded-md px-5 py-3 max-h-64 overflow-y-scroll">
                                <template v-if="payload.processes.length">
                                    <div v-for="(process, index) in payload.processes" :key="index" class="w-full flex grid grid-cols-2 gap-2 text-sm my-3">
                                        <div>
                                            <p>{{ process }}</p>
                                        </div>
                                        <div class="text-right">
                                            <button @click="removeProcess(index)" type="button" class="text-red-600"><font-awesome-icon icon="fa-solid fa-minus" /></button>
                                        </div>
                                    </div>
                                </template>
                                <template v-else>
