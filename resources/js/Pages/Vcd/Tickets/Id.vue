...
export default {
    ...
    setup(props) {
        ...
        const { staffNote, staffCompleted, staffTransfer } = useTickets()
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <Breadcrumb parentTitle="Tickets" :parentLink="route('tickets.index')" :currentTitle="'View #' + item.id" />
        </template>
                    ...
                    <div class="w-full py-5 px-7">
                        <div class="w-full bg-gray-100 h-48 rounded-md shadow-md flex flex-wrap mt-2 mb-5 p-5 overflow-y-scroll">
                            <template v-if="item.notes.length">
                                <template v-for="note in item.notes" :key="note.id">
            ...
            <div class="w-full flex flex-wrap">
                ...
                <div class="w-full md:w-1/3 my-2">
                    <div class="w-full flex grid grid-cols-3 gap-3">
                        ...
                        <button @click="completedTicket(item.id)" type="button" class="bg-up-green hover:bg-up-green-darker text-white py-1 rounded-md">Completed</button>
                        <button @click="toggleTransfer()" type="button" class="bg-blue-600 hover:bg-blue-500 text-white py-1 rounded-md">Transfer</button>
                    </div>
                </div>
            </div>
            <!-- End Page header -->
